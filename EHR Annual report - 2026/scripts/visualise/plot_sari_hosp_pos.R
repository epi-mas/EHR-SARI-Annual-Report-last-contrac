...
for (country in country_codes) {
  ...
  # Convert positivity columns to numeric to avoid pivot_longer() errors
  if (length(positivity_data) > 0) {
    df <- df %>%
  ...
  plot <- ggplot(df, aes(x = week)) +
    geom_bar(aes(y = .data[[hospitalization_var]], fill = "SARI Cases"),
             stat = "identity", alpha = 0.7) +  # Ensures SARI Cases appears in legend
    scale_fill_manual(
      values = c("SARI Cases" = "grey"),
  ...
  # Save plot
  file_path <- file.path(output_dir, paste0("SARI_plot_", country, "_", Sys.Date(), ".png"))
